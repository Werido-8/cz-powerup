...
function ActionBtn({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex shrink-0 items-center gap-1 whitespace-nowrap rounded-md px-2 py-1 text-[12px] transition-colors ${cls}`}
    >
...
export function usePaperQuestionGroups(initialGroups: EditorGroup[] = EDITOR_GROUPS) {
  ...
  const resetGroups = () => setGroups(structuredClone(initialGroups));
  ...
  return { groups, collapsed, toggleCollapse, move, remove, aiAppend, resetGroups, summary };
}
          ...
          <div key={g.type} className="rounded-lg border border-border bg-card">
            <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-2.5">
              ...
              <div className="flex items-center gap-0.5">
                ...
                <ActionBtn icon={Sparkles} label="AI 补题" tone="primary" onClick={() => onAiAppend(g.type)} />
              </div>
            </div>
            {!isCol && (
