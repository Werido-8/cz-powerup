  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col">
      <header className="flex h-[22px] shrink-0 items-center justify-between gap-2">
        ...
        <span className="text-[11.5px] tabular-nums text-kb-muted">{COMPARE_CHAPTER_COUNT} 章</span>
      </header>

      <div className="mt-2.5 flex h-8 shrink-0 items-center gap-1.5 rounded-[6px] border border-kb-border bg-white px-2.5 transition-colors focus-within:border-primary/60 focus-within:ring-1 focus-within:ring-primary/20">
      ...
      <div className="scrollbar-thin mt-2.5 min-h-0 flex-1 overflow-y-auto pr-0.5">
        <OutlineRow
          label="全文概览"
          count={totalCount}
...
function OutlineRow({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "flex h-[29px] w-full items-center justify-between gap-2 rounded-[5px] pr-2.5 text-left transition-colors",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/25",
        nested ? "pl-6" : "pl-2.5",
        active ? "bg-primary-soft font-medium text-primary" : "text-kb-body hover:bg-[#F5F9FA]",
      )}
    >
