...
function ParseStatusTag({ status }: { status: KbParseStatus }) {
  ...
  return (
    <span className={cn("shrink-0 rounded-md px-1.5 py-0.5 text-[10px] font-medium ring-1", styles[status])}>
...
function FileTypeThumb({ file }: { file: KbFile }) {
  return <KbFileTypeIcon type={file.fileType} fileName={file.name} size="lg" />;
}
      ...
      <div className="flex flex-1 flex-col overflow-y-auto">
        <div className="flex flex-col gap-1.5 p-3">
          {files.map((file) => (
            ...
            >
              <FileTypeThumb file={file} />
              <div className="min-w-0 flex-1">
                <div className="truncate text-[13px] font-semibold text-foreground">{file.name}</div>
                <div className="mt-0.5 truncate text-[11.5px] text-muted-foreground">{file.summary}</div>
              </div>
      ...
      {files.map((file) => (
        ...
        >
          ...
          <div className="flex gap-2.5">
            <FileTypeThumb file={file} />
            <div className="min-w-0 flex-1">
              <h3 className="line-clamp-2 text-[12.5px] font-semibold leading-snug text-foreground">{file.name}</h3>
              <p className="mt-1 line-clamp-2 text-[11px] leading-relaxed text-muted-foreground">{file.summary}</p>
            </div>
          </div>
